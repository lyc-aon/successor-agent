...
SuccessorDemo  — full keyframe sequence with Bayer-dithered morphs
SuccessorShow  — render a single static braille frame in the same chrome
...
# ─── Palette (warm red on near-black) ───
INK_DEEP = 0x10070A
INK_BLOOD = 0xC1272D
INK_EMBER = 0xFF6347
INK_BONE = 0xE6D9B8
...
def _paint_chrome(grid: Grid, *, subtitle: str, status_left: str, status_right: str) -> tuple[int, int]:
    ...
    title = "  successor  ·  terminal renderer  "
...
def _paint_successor_block(grid: Grid, lines: list[str], top: int, bottom: int) -> None:
...
# ─── SuccessorDemo: animated keyframe sequence with viewport scaling ───


class SuccessorDemo(App):
    def __init__(
        self,
        *,
    ...
    def on_tick(self, grid: Grid) -> None:
        ...
        _paint_successor_block(grid, frame_lines, top, bottom)


# ─── SuccessorShow: single static frame, viewport-scaled ───


class SuccessorShow(App):
    ...
    def on_tick(self, grid: Grid) -> None:
        ...
        left = f" successor · show {self.pose_name} "
        ...
        _paint_successor_block(grid, lines, top, bottom)

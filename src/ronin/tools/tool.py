...
Tools are the only Ronin extension type that turns data into code. The
discipline:

  - Built-in tools live in `src/ronin/builtin/tools/*.py`. They are
    reviewed at commit time and load by default.

  - User tools live in `~/.config/ronin/tools/*.py`. They are GATED
...
# do `from ronin.tools import tool` and then `@tool(...)` without
...
class ToolRegistry:
    ...
    Walks two directories — `src/ronin/builtin/tools/` and (when
    enabled) `~/.config/ronin/tools/` — importing each `*.py` file in
    ...
    def _import_one(self, path: Path, *, source: str) -> None:
        if source == "user":
            print(
                f"ronin: loading user tool {path}",
                file=sys.stderr,
            )

        # Use a unique module name so multiple imports of the same path
        # don't collide in sys.modules. Tools loaded by ronin live in
        # the synthetic `_ronin_tools` namespace.
        mod_name = f"_ronin_tools.{source}.{path.stem}"

...
Six places in Ronin need the same loading shape:

  themes      JSON files in builtin/themes/ and ~/.config/ronin/themes/
  profiles    JSON files in builtin/profiles/ and ~/.config/ronin/profiles/
  skills      Markdown files in builtin/skills/ and ~/.config/ronin/skills/
  tools       Python files in builtin/tools/ and ~/.config/ronin/tools/
  intros      Frame directories in builtin/intros/ and ~/.config/ronin/intros/
...
1. Loader is hermetic-testable via the same `RONIN_CONFIG_DIR` env var that
...
# MUST stay in sync — both honor RONIN_CONFIG_DIR for hermetic tests.
...
def config_dir() -> Path:
    """Resolve the user config directory, honoring $RONIN_CONFIG_DIR."""
...
def builtin_root() -> Path:
    ...
    Walks up from this file (`src/ronin/loader.py`) to find `src/ronin/`,
...
class Registry(Generic[T]):
    ...
    inside the package and a user dir under `~/.config/ronin/`. The two
    ...
    def __init__(
        ...
        *,
        ...
        # Useful for `rn themes list` etc to show source labels.
    ...
    def _scan_dir(self, directory: Path, *, source: str) -> None:
        ...
        # into subdirs because Ronin's registries are flat by design.
    ...
    def _warn(self, message: str) -> None:
        ...
        print(f"ronin: {message}", file=sys.stderr)

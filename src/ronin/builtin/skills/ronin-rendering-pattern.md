...
description: Use this skill when extending or debugging the Ronin terminal renderer, when adding any visual feature to a chat-shaped TUI, or when tempted to import Rich, prompt_toolkit, Textual, or any other library that wants to own stdout.
---

# Ronin Rendering Pattern
...
**One module owns stdout.** In Ronin, that's `src/ronin/render/diff.py`.
...
each assumed they owned stdout. Ronin eliminates that war by having
...
cache, never re-parses. Validated speedups in Ronin: 16× for
...
- `~/dev/ai/ronin/docs/rendering-superpowers.md` — full architectural rules
- `~/dev/ai/ronin/docs/concepts.md` — features the architecture enables
- `~/dev/ai/ronin/docs/rendering-plan.md` — original five-layer decisions

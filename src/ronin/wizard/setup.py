"""RoninSetup — the profile creation wizard.
...
Layout (resize-aware, all coordinates derived from grid.rows/cols):
  ...
  │              │  live preview ─────────────────────────────      │
  │              │  ┌──────────────────────────────────────┐        │
  │              │  │ ronin · chat        normal ☾ ◆ steel │        │  preview pane
  │              │  │                                      │        │  (sub-rendered)
  │              │  │ ronin                                │        │
  │              │  │ I am ronin. The forge is hot.        │        │
  │              │  └──────────────────────────────────────┘        │
  │              │                                                  │
  ├──────────────┴──────────────────────────────────────────────────┤
...
Renderer capabilities exercised (concepts.md categories):
  ...
  Cat 1 — Mutable cells: the preview pane is a live sub-rendered
          RoninChat that updates every frame as the user picks options.
...
_DEFAULT_SYSTEM_PROMPT = (
    ...
    "Speak as ronin would: with intention, with brevity, as if seated by "
    "a wayside fire speaking to a fellow traveler. Reply in a single flowing "
    "paragraph.\n\n"
...
class _WizardState:
    ...
    def to_profile(self) -> Profile:
        ...
        return Profile(
            ...
            description=f"created via rn setup",
    ...
    def to_json_dict(self) -> dict:
        ...
        return {
            "name": self.name.strip().lower() or "untitled",
            "description": "created via rn setup",
            "theme": self.theme_name,
            "display_mode": self.display_mode,
            "density": self.density,
            "system_prompt": _DEFAULT_SYSTEM_PROMPT,
...
def _try_load_welcome_frame() -> BrailleArt | None:
    ...
    # walk up from src/ronin/wizard/setup.py to the repo root
...
class RoninSetup(App):
    ...
    def __init__(
        ...
        *,
        ...
        # construct and run RoninChat with `saved_profile`. False means
    ...
    def _build_preview_chat(self):
        """Build a fresh RoninChat configured to render the wizard's preview.
        ...
        """
        from ..demos.chat import RoninChat, _Message
        ...
        chat.messages.append(
            _Message(
                "ronin",
                "Greetings, traveler. I am ronin — the blade rests; "
    ...
    def _paint_theme(
        ...
        if not themes:
            paint_text(
                grid, "(no themes loaded — drop *.json into ~/.config/ronin/themes/)",
                left, top,
    ...
    def _paint_live_preview(
        ...
        *,
        ...
        RoninChat that we mutate as the user picks options. The chat's

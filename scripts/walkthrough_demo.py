...
def run_walkthrough_showcase(driver: Driver) -> None:
    ...
    # active profile / provider / context window. Tightened from
    # 5s → 3s; the panel is static so 3s is plenty to read.
    driver.section(1, "empty-state hero panel")
    driver.pump_for(3.0)

    # ─── Prompt 1: list-directory verb ───
    # `ls -la /tmp` is safe, universal, and produces enough output
    # to make the tool card look full. Counting items forces a
    # continuation turn after the bash result comes back.
    ...
    press_enter(driver)
    driver.pump_for(15.0)

    # ─── Prompt 2: read-file verb ───
    # README.md is in the repo root. The continuation summarizes
    # the file contents — shows the model actually reading the
    # tool output.
    ...
    # turn between them. Pump still longer than the others because
    # of the two-tool-call agent loop.
    ...
    press_enter(driver)
    driver.pump_for(28.0)

    # ─── Search demo ───
    # Now that we have 6 prompts of model output + 6 tool cards in
    # the chat history, demonstrate the Ctrl+F search across the
    # whole scrollback. The harness's custom scrollback can highlight
    # matches in mutable in-memory messages — something other
    # harnesses can't do because once they `print()` a line, it
    # belongs to the terminal scrollback and can't be re-styled.
    driver.section(14, "search demo — Ctrl+F to open")
    press_ctrl(driver, "f")
    driver.pump_for(2.0)

    # Type a query that matches multiple turns ('successor' appears
    # in the prompts, the tool cards, AND the model replies)
    driver.section(15, "search query — type 'successor'")
    type_string(driver, "successor")
    driver.pump_for(2.5)

    # Navigate matches — Down jumps to next, gives the viewer time
    # to see the highlighted match move through the history
    driver.section(16, "next match — Down arrow")
    press_down(driver)
    driver.pump_for(2.0)

    driver.section(17, "next match — Down again")
    press_down(driver)
    driver.pump_for(2.0)

    driver.section(18, "previous match — Up arrow")
    press_up(driver)
    driver.pump_for(2.0)

    # Esc closes search and returns to the input
    driver.section(19, "Esc — close search")
    press_esc(driver)
    driver.pump_for(1.5)

    # ─── Clean exit ───
    driver.section(20, "type /quit")
    type_string(driver, "/quit")
    driver.pump_for(0.3)

    driver.section(21, "submit /quit → exit")
    press_enter(driver)
    driver.pump_for(1.0)

    driver.section(22, "walkthrough complete")
